...
function setupGateway() {
  ...
  setTimeout(
    () => send('AT+QIOPEN=1,0,"TCP LISTENER","0.0.0.0",0,5000,2'),
    9000,
  );
}

port.on("open", () => {
  console.log(`\x1b[32m[SUCCESS]: COM13 Connected.\x1b[0m`);
  setupGateway();
});
...
parser.on("data", (line) => {
  ...
  if (raw.includes("+QIOPEN: 0,562") || raw.includes("+QIOPEN: 0,572")) {
    console.log(
      "\x1b[31m[ERROR]: Socket Busy or Context Error. Retrying in 5s...\x1b[0m",
    );
  ...
  if (raw.includes("+QIOPEN: 0,0")) {
    isNetworkReady = true;
    console.log(
      "\x1b[42m\x1b[30m SYSTEM LIVE \x1b[0m Listening for Assets on Port 5000...",
    );
  }

  // --- 3. DATA ARRIVAL (The "RECV" URC) ---
  // When the modem says +QIURC: "recv",X it means data is in the buffer
  const recvMatch = raw.match(/\+QIURC:\s*"recv",(\d+)/);
  if (recvMatch) {
    const socketId = recvMatch[1];
    console.log(
      `\x1b[35m[INCOMING]: Data detected in buffer on socket ${socketId}. Fetching...\x1b[0m`,
    );
    // Command the modem to read the data (max 1500 bytes)
    send(`AT+QIRD=${socketId},1500`);
  }
